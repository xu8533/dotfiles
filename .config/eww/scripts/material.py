...
def setup(img):
    ...
    os.system(
        "gradience-cli apply -p ~/.config/eww/scripts/colors/colors-gradience.json --gtk both"
    )
    os.system(
        f"swww img {WALLPAPER_PATH} --transition-fps 75 --transition-type wipe --transition-duration 2"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image", type=str, help="Generate color scheme based on an image file."
    )
    parser.add_argument(
        "--toggle",
        action="store_true",
        help="Toggle between light and dark color schemes.",
    )
    parser.add_argument(
        "--current", action="store_true", help="Print current colors scheme(light/dark)"
    )
    parser.add_argument(
        "--color",
        type=str,
        help="Generate color scheme based on a color and simple plain wallpaper",
    )
